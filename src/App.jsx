        ...
        <div style={{ background: 'rgba(10,10,10,0.95)', padding: '50px 20px', borderRadius: '35px', border: '2px solid #FFD700', width: '380px', textAlign: 'center', marginBottom: '20px' }}>
          ...
          <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '15px' }}>
            <input 
              type="text" 
              placeholder="Usuario" 
              style={{ width: '85%', padding: '14px 18px', background: '#e8f0fe', border: 'none', color: '#000', borderRadius: '12px', outline: 'none', fontSize: '16px' }} 
            ...
            <button style={{ width: '90%', padding: '15px', background: '#FFD700', color: '#000', fontWeight: '900', borderRadius: '12px', border: 'none', cursor: 'pointer', fontSize: '16px', marginTop: '15px' }}>
              ENTRAR
            </button>
          </form>
        </div>
        <p style={{ color: '#fff', fontSize: '11px', fontWeight: 'bold', letterSpacing: '0.5px', opacity: 0.8 }}>
    ...
    <div style={{ backgroundColor: '#050505', minHeight: '100vh', display: 'flex', flexDirection: 'column', color: '#fff', padding: '20px', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ flex: 1 }}>
        ...
        <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#111', padding: '15px 25px', borderRadius: '15px', border: '1px solid #222', marginBottom: '20px' }}>
          ...
          <button onClick={() => setIsLoggedIn(false)} style={{ background: 'none', border: '1px solid #FFD700', color: '#FFD700', padding: '8px 15px', borderRadius: '10px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', fontWeight: 'bold' }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed' }}>
            <thead>
              <tr style={{ background: '#000', color: '#FFD700', borderBottom: '2px solid #FFD700' }}>
                ...
                {nombresDias.map((d, i) => (
                  <th key={i} style={{ width: '90px', fontSize: '12px' }}>{d} {numerosDias[i]}</th>
                ))}
              </tr>
              ...
              {empleadosVisibles.map(emp => {
                ...
                return (
                  <tr key={id} style={{ borderBottom: '1px solid #111' }}>
                    ...
                    <td style={{ textAlign: 'center', color: '#aaa', fontSize: '12px', fontWeight: '600' }}>{emp.Sede}</td>
